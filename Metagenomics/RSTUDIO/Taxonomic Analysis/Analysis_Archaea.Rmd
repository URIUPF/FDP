...
#Load data
data <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/ARCHAEA/DATA_ARCHAEA.csv") # metadata + kraken_data
# MAYBE I HAVE TO SET "D55" == "D49"

metadata <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/ARCHAEA/metadata_ARCHAEA.csv")
metadata <- unique(metadata)
kraken_data <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/ARCHAEA/report_data_ARCHAEA.csv")
...
archaea <- data[data$Taxon_name == "Archaea", ] # D
archaea <- unique(archaea)
archaea <- na.omit(archaea)

data <- data[data$Taxon_name != "Archaea", ]
data <- data[data$Taxon_name != "cellularorganisms", ]
data <- unique(data)
```

```{r}
marchaea <- mean(archaea$Percent)
marchaea
```
```{r}
...
aggregated_data <- aggregate(Count ~ Sample + Taxon_name, data = data_alpha, sum) # Count
...
alpha_div_dataset <- alpha_div_dataset[-nrow(alpha_div_dataset), ]
...
V1
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Archaea/Archaea_BoxplotAlpha.svg", V1, height = 6, width = 10)

kruskal2 <- kruskal.test(alpha_div_dataset$alpha_div ~ alpha_div_dataset$Group)
print(kruskal2) # the p-value is 0.08, which is greater than 0.05 == is no significant difference in Shannon diversity between groups based on the Kruskal-Wallis test. 
...
pcoa_day21 <- pcoa %>%
  filter(Day %in% "21")
a_d21 <- ggplot(data = pcoa_day21, aes(x = pcoa_day21[, 2], y = pcoa_day21[, 3], col = Group)) +
...
a_d21
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Archaea/Archaea_PCoA_D21.png", a_d21, height = 6, width = 10)



pcoa_day49 <- pcoa %>%
  filter(Day %in% "49")
a_d49 <- ggplot(data = pcoa_day49, aes(x = pcoa_day49[, 2], y = pcoa_day49[, 3], col = Group)) +
  geom_point(aes(col = Group)) + 
  labs(x = "PC1", y = "PC2", color = "Group") +
  scale_color_manual(name = "Group", values = c("blue", "green", "red", "purple", "orange")) + # values = color_vector,
  scale_fill_manual(name = "Group", values = c("blue", "green", "red", "purple", "orange")) + # values = color_vector,
...
a_d49
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Archaea/Archaea_PCoA_D49.png", a_d49, height = 6, width = 10)
...
perm_result
# This performs the PERMANOVA test. Here, Group is the factor to test against. The permutations = 999 argument specifies the number of permutations to use for the test.
```
...
permanova_21 # 0,7 -_> NO statistically different beta diversity at DAY 21
...
permanova_49 # p > 0.05 --> NO statistically significant DIFFERENCES between groups at day 49
...
plot_taxa_distribution <- function(data, ranks, variable) {
  ...
  ggplot(data_filtered, aes(x = Group, y = !!sym(variable), fill = Taxon_name)) +
    ...
    labs(x = 'Group', y = y_label, title = paste('Archaeal Taxa Distribution (' ,ranks, ') per Group')) +
    theme_bw() +
...
taxa_per_group
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Archaea/Arch_Taxa_per_Group.png", taxa_per_group, height = 6, width = 10)
```
