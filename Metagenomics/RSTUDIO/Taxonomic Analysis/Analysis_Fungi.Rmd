...
#Load data
data <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/FUNGI/DATA_FUNGI.csv") # metadata + kraken_data
# MAYBE I HAVE TO SET "D55" == "D49"

metadata <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/FUNGI/metadata_FUNGI.csv")
kraken_data <- read.csv("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/FUNGI/report_data_FUNGI.csv")
...
fungi <- data[data$Taxon_name == "Fungi", ] # is a Kingdom
fungi <- unique(fungi)
fungi <- na.omit(fungi)
mfungi <- mean(fungi$Percent)
mfungi
...
f1 <- ggplot(alpha_div_dataset, aes(x = Group, y = alpha_div, fill = Group)) +
  geom_boxplot() +
  labs(
    # title = "Alpha diversity (Shannon index) in FUNGI at different groups", 
       x = "Groups", y = "Shannon Diversity") +
  scale_fill_manual(name = "Group", values = c("green", "orange", "red", "blue", "purple")) + # ADJUST MANUALLY
  theme_classic()
f1
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Fungi/Alpha_FUNGI.svg", f1, height = 6, width = 10)


kruskal2 <- kruskal.test(alpha_div_dataset$alpha_div ~ alpha_div_dataset$Group)
print(kruskal2) # the p-value is 0.03, which is lower than 0.05 == is significant difference in Shannon diversity between groups based on the Kruskal-Wallis test. 
...
pcoa_day21 <- pcoa %>%
  filter(Day %in% "21")
f_d21 <- ggplot(data = pcoa_day21, aes(x = pcoa_day21[, 2], y = pcoa_day21[, 3], col = Group)) +
...
f_d21
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Fungi/Fungi_PCoA_D21.png", f_d21, height = 6, width = 10)



pcoa_day49 <- pcoa %>%
  filter(Day %in% "49")
f_d49 <- ggplot(data = pcoa_day49, aes(x = pcoa_day49[, 2], y = pcoa_day49[, 3], col = Group)) +
  geom_point(aes(col = Group)) + 
  labs(x = "PC1", y = "PC2", color = "Group") +
  scale_color_manual(name = "Group", values = c("blue", "green", "red", "purple", "orange")) + # values = color_vector,
  scale_fill_manual(name = "Group", values = c("blue", "green", "red", "purple", "orange")) + # values = color_vector,
...
f_d49
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Fungi/Fungi_PCoA_D49.png", f_d49, height = 6, width = 10)
...
permanova_21 # 0.3 -_> NO statistically different beta diversity at DAY 21
...
plot_taxa_distribution <- function(data, ranks, variable) {
  ...
  ggplot(data_filtered, aes(x = Group, y = !!sym(variable), fill = Taxon_name)) +
    ...
    labs(x = 'Group', y = "Abundance", title = paste('Fungal Taxa Distribution (' ,ranks, ') per Group')) +
    theme_bw() +
...
taxa_per_group
ggsave("/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/REPORT AND FDP/Figures/SuppMat/Fungi/Fungi_Taxa_per_Group_.png", taxa_per_group, height = 6, width = 10)
```
