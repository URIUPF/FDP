...
scp -P 2122 mblanco@hpc.csuc.cat:'/scratch/mblanco/baro/metagenomic/analysis/URI_3_kraken/*.report_P'
```{r}
# Directory containing Kraken reports
kraken_reports_dir <- "/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/VIRUS/--meta"
...
process_kraken_report <- function(report_file) {
  ...
  # total_abundance <- sum(df$Count)
  # 
  # # Calculate adjusted abundances
  # df$Adjusted_Abundance <- (df$Count / total_abundance) * 100
  # #####################################
...
report_files <- list.files(kraken_reports_dir, pattern = "\\.report_V$", full.names = TRUE)
...
# FOR EACH REPORT, I HAVE TO PICK ALL TAXON NAMES THAT BELONG TO THAT REPORT AND DIVIDE IT BY THE PERCENT OF "Viruses" OF THE REPORT
...
```{r}
# Calculate adjustment factor for each sample
adjustment_factors <- merged_data %>%
  filter(Taxon_name == "Viruses") %>%
  group_by(Sample) %>%
  summarise(adjustment_factor = 1 / Percent)

# Merge adjustment factors with the original data
merged_data <- merged_data %>%
  left_join(adjustment_factors, by = "Sample") %>%
  mutate(adjusted_percent = ifelse(Taxon_name == "Viruses", Percent, Percent * adjustment_factor))

# Drop adjustment_factor column if no longer needed
merged_data <- merged_data %>% select(-adjustment_factor)
```

# SAVE DATAFRAMES IN LOCAL
```{r}
write.csv(combined_metadata, "/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/VIRUS/--careful/DATA_R/metadata.csv", row.names = FALSE)
write.csv(reports_df, "/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/VIRUS/--careful/DATA_R/report_data.csv", row.names = FALSE)
write.csv(merged_data, "/home/uriii/Desktop/BIOINFORMATICS/INT_FDG/ANALYSIS/DATA/Metagenomics/KRAKEN/VIRUS/--careful/DATA_R/DATA.csv", row.names = FALSE)

# write.csv(combined_metadata, "DATA_R/--careful/metadata.csv", row.names = FALSE)
# # write.csv(reports_df, "DATA_R/--careful/report_data.csv", row.names = FALSE)
# # write.csv(merged_data, "DATA_R/--careful/DATA.csv", row.names = FALSE)

# write.csv(combined_metadata, "NewABUNDANCES/--meta/metadata.csv", row.names = FALSE)
# write.csv(reports_df, "NewABUNDANCES/--meta/report_data.csv", row.names = FALSE)
# write.csv(merged_data, "NewABUNDANCES/--meta/DATA.csv", row.names = FALSE)
```
